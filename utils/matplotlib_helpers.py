...
def draw_subpcolor(data, legend, filename, clim=[-60, 20], fs=16e3, wlen=1024, shift=256):

    colors = [(1,1,1), (1,1,1), (0,0,1)]              # W -> W -> B
    ...
    #create empty subplots
    fig, axes = plt.subplots(nrows=len(data), ncols=1)
    fig.set_size_inches(10, min(3*len(data),20))

    #use vertical padding of 2x the font size between the subplots to avoid overlapping labels
    fig.tight_layout(h_pad=2.5, rect=(0.05, 0.01, 0.99, 0.99))

    #each legend,data tuple goes into a subplot
    for i, x in enumerate(data):
        ax = plt.subplot(len(data), 1, i+1)
        tvect = np.linspace(0, x.shape[0]*shift/fs, x.shape[0]+1)
        fvect = np.linspace(0, fs*0.5, x.shape[1]+1)
        plt.pcolor(tvect, fvect, x.T, cmap=cmap)
        plt.title(legend[i])
        ax.set_xlim([0, tvect[-1]])
        ax.set_ylim([0, fvect[-1]])
        if i==(len(data)-1):
            plt.xlabel('time [s]')
